...
builder.Services.AddDbContext<CommandModelDatabase>(options => options.UseSqlServer(connectionString));
builder.Services.AddDbContext<QueryModelDatabase>(options => options.UseSqlServer(connectionString));
...
{ 
    options.ViewLocationExpanders.Add(new MultiDeviceViewLocationExpander());
});
...
builder.Services.AddScoped<ICommandHandler<ProcessOrderBeforePaymentCommand, OrderProcessingViewModel>, ProcessOrderBeforePaymentHandler>();
builder.Services.AddScoped<ICommandHandler<ProcessOrderAfterPaymentCommand, OrderProcessedViewModel>, ProcessOrderAfterPaymentHandler>();
...
if (app.Environment.IsDevelopment())
{
    ...
    var db = sp.ServiceProvider.GetRequiredService<CommandModelDatabase>();
    //db.Database.EnsureDeleted();
    if (db.Database.EnsureCreated())
    {
        SampleAppInitializer.Seed(db, 
            sp.ServiceProvider.GetRequiredService<IHashingService>(),
            g => IdentityHelpers.GetAvatar(g),
            sp.ServiceProvider.GetRequiredService<IOrderRequestService>().GenerateTemporaryOrderId);
    }
}
else
{
